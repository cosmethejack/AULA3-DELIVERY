...
Fundação do monorepo e do ambiente de desenvolvimento: organização dos apps (`apps/backend`, `apps/frontend`), orquestração de scripts pela raiz, configuração de TypeScript, ESLint e runners de teste (Jest, Vitest, Playwright) por app, ambiente único de configuração (`.env` na raiz) e banco local via `docker-compose`.

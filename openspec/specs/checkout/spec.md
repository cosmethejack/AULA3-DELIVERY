...
Fluxo de checkout do frontend: carrinho com persistência local, formulário de dados do cliente e confirmação do pedido.

## Requirements

### Requirement: Carrinho com Persistência Local
O carrinho DEVE (MUST) ser gerenciado via `CartContext` com persistência em localStorage. Itens, quantidades e total são mantidos entre sessões.
...
O checkout DEVE (MUST) coletar nome, endereço, e-mail e telefone do cliente antes de confirmar o pedido.
...
Após checkout bem-sucedido, o sistema DEVE (MUST) redirecionar para página de confirmação com resumo do pedido.

...
O dashboard DEVE (MUST) exibir sidebar escura com links para Pedidos, Produtos, Categorias, Clientes e Resumo. A sidebar colapsa em tablet.
...
O dashboard DEVE (MUST) permitir criar, listar, editar e remover categorias e produtos via `DataTable` com paginação.
...
O dashboard DEVE (MUST) listar pedidos com filtro por status e permitir avançar o status de cada pedido.
...
A página /dashboard/resumo DEVE (MUST) exibir StatsCards com vendas totais, recebido, pendente e gráfico de barras.

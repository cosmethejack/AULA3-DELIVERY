...
Plataforma core do backend NestJS: fundamentos transversais sobre os quais os módulos de negócio são plugados — versionamento de rota `/v1`, `ValidationPipe` global, erros no formato RFC 9457 (Problem Details) via filtro global único, `ConfigModule` global validado no boot (fail-fast), documentação OpenAPI/Swagger em `/docs` e endpoint de health.

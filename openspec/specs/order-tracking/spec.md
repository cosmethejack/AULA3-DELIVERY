...
Acompanhamento de pedidos pelo cliente: listagem de pedidos, timeline visual dos estados e alerta de atraso com cupom.

## Requirements

### Requirement: Listagem de Pedidos do Cliente
A página `/pedidos` DEVE (MUST) listar os pedidos do cliente autenticado com status, data e valor total.
...
A página `/pedidos/[id]/acompanhamento` DEVE (MUST) exibir timeline vertical com os 6 estados do pedido, destacando o estado atual.
...
O sistema DEVE (MUST) exibir `DelayAlert` com cupom quando o pedido exceder o prazo estimado de entrega.
